...
for s = 1:size(session_list,1)
    ...
    %events in seconds
%     ntime = size(lfp,1);
    timevec_seconds= -(15*60):5:(20*60);
    LOC = sessionInfo.eyesClose(1);
    drugStart = sessionInfo.drugStart(1)-LOC;
    drugStart2 = sessionInfo.drugStart(2)-LOC;
    drugEnd = sessionInfo.drugEnd(2)-LOC;
    ROC = sessionInfo.eyesOpen(1)-LOC;
...
%                 end
            ...
            for c = 1:length(PFCindx)
                spike_rates{1}(c,t) = length(find(spikeTimes{PFCindx(c)}-LOC<=latency(end) & spikeTimes{PFCindx(c)}-LOC>=latency(1)))/sniplen;
            end

            %FEF
            for c = 1:length(FEFindx)
                spike_rates{2}(c,t) = length(find(spikeTimes{FEFindx(c)}-LOC<=latency(end) & spikeTimes{FEFindx(c)}-LOC>=latency(1)))/sniplen;
            end
            %PPC
            for c = 1:length(PPCindx)
                spike_rates{3}(c,t) = length(find(spikeTimes{PPCindx(c)}-LOC<=latency(end) & spikeTimes{PPCindx(c)}-LOC>=latency(1)))/sniplen;
            end
            %STG
            for c = 1:length(STGindx)
                spike_rates{4}(c,t) = length(find(spikeTimes{STGindx(c)}-LOC<=latency(end) & spikeTimes{STGindx(c)}-LOC>=latency(1)))/sniplen;
            end
            t
...
%     xline(ROC, 'g', 'LineWidth', 2)
    ...
    save([sessionInfo.session '_SUA_LOC_Locked_NoPuffTones'], 'drugStart', 'drugStart2', 'drugEnd', 'LOC', 'ROC', 'timevec_seconds', 'sessionInfo', 'spike_rates', '-v7.3')
...
filestoload = ls('*_SUA_LOC_Locked_NoPuffTones*')
timevec_seconds= -(15*60):5:(20*60);
...
xline(mean(drugstart_sessions), 'k', 'LineWidth', 2)
xline(mean(drugstart_sessions)-std(drugstart_sessions)./sqrt(size(drugstart_sessions,1)), 'k', 'LineWidth', 1)
xline(mean(drugstart_sessions)+std(drugstart_sessions)./sqrt(size(drugstart_sessions,1)), 'k', 'LineWidth', 1)
xline(mean(drugstart2_sessions), 'k', 'LineWidth', 2)
xline(mean(drugstart2_sessions)-std(drugstart2_sessions)./sqrt(size(drugstart2_sessions,1)), 'k', 'LineWidth', 1)
xline(mean(drugstart2_sessions)+std(drugstart2_sessions)./sqrt(size(drugstart2_sessions,1)), 'k', 'LineWidth', 1)
xlim([-15 20.05])
ylim([0 10])
...
LOCbin = nearest(newtimevec, 0)
meanFRLOC = zeros(4,1);
meanFRLOC(1) = nanmean(spikes_smdn{1}(:,LOCbin))
meanFRLOC(2) = nanmean(spikes_smdn{2}(:,LOCbin))
meanFRLOC(3) = nanmean(spikes_smdn{3}(:,LOCbin))
meanFRLOC(4) = nanmean(spikes_smdn{4}(:,LOCbin))

meanCIFRLOC = zeros(4,2);
[confInts] = bootstrapConfIntervals(spikes_smdn{1}(:,LOCbin), @nanmean, [],0.99,10000);
meanCIFRLOC(1,1) = confInts(1); 
meanCIFRLOC(1,2) = confInts(2); 
[confInts] = bootstrapConfIntervals(spikes_smdn{2}(:,LOCbin), @nanmean, [],0.99,10000);
meanCIFRLOC(2,1) = confInts(1); 
meanCIFRLOC(2,2) = confInts(2); 
[confInts] = bootstrapConfIntervals(spikes_smdn{3}(:,LOCbin), @nanmean, [],0.99,10000);
meanCIFRLOC(3,1) = confInts(1); 
meanCIFRLOC(3,2) = confInts(2); 
[confInts] = bootstrapConfIntervals(spikes_smdn{4}(:,LOCbin), @nanmean, [],0.99,10000);
...
spike_rates_CAT = cat(1, spike_rates_sessions{1}, spike_rates_sessions{2}, spike_rates_sessions{3},spike_rates_sessions{4});

LOCbin = nearest(timevec_minutes, 0)
meanFRLOC = mean(mean(spike_rates_CAT(:,LOCbin),2));
semFRLOC = std(mean(spike_rates_CAT(:,LOCbin),2))./size(spike_rates_CAT,1);


LOCbin = nearest(timevec_minutes, 15)
meanFRLOC = mean(mean(spike_rates_CAT(:,LOCbin),2));
semFRLOC = std(mean(spike_rates_CAT(:,LOCbin),2))./size(spike_rates_CAT,1);


figure; 
shadedErrorBar(timevec_minutes, mean(spike_rates_sessions{1}), spike_sem(1,:), 'b'); hold on;
...
mean(mean(spike_rates_sessions{1}(:,LOCbin),2))
mean(mean(spike_rates_sessions{2}(:,LOCbin),2))
mean(mean(spike_rates_sessions{3}(:,LOCbin),2))
mean(mean(spike_rates_sessions{4}(:,LOCbin),2))

[p h ] = ranksum( mean(spike_rates_sessions{1}(:,LOCbin),2), mean(spike_rates_sessions{3}(:,LOCbin),2))
[p h ] = ranksum( mean(spike_rates_sessions{1}(:,LOCbin),2), mean(spike_rates_sessions{4}(:,LOCbin),2))
[p h ] = ranksum( mean(spike_rates_sessions{2}(:,LOCbin),2), mean(spike_rates_sessions{3}(:,LOCbin),2))
[p h ] = ranksum( mean(spike_rates_sessions{2}(:,LOCbin),2), mean(spike_rates_sessions{4}(:,LOCbin),2))
